...
interface KeyInfo { key: string; bytes: string; sequence: string; desc: string }
...
export function KeyboardDemo() {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    const keyInfo: KeyInfo = special
      ? { key: e.key, bytes: special.bytes, sequence: special.sequence, desc: special.desc }
      : { key: e.key, bytes: e.key.charCodeAt(0).toString(16).padStart(2, "0"), sequence: e.key, desc: `Character '${e.key}'` };
        ...
        <div className="min-h-[200px] flex flex-col items-center justify-center cursor-text" onClick={() => inputRef.current?.focus()}>
          ...
          {lastKey ? (
            <div className="text-center space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="bg-terminal-bg rounded p-3">
                  ...
                  <code className="text-terminal-cyan text-lg">{lastKey.sequence}</code>
                </div>
              </div>
            ...
            <div className="text-terminal-dim text-center">
              ...
              <div className="text-sm">Try arrow keys, Enter, Tab, or regular letters</div>
            </div>
          )}
        </div>
      </TerminalWindow>
