...
const EXPLAINER_STEPS: Record<ExplainerStep, StepContent> = {
  overview: {
    ...
    description: "Advanced TUIs divide the terminal into regions. Each region is a rectangular area with its own content and borders. The TUI framework tracks each region's position and size in the character grid.",
    highlightRegions: ["tabs", "sidebar", "content"],
  },
  "layout-system": {
    title: "Region Coordinates",
    description: "Each region stores its position (x, y) and dimensions (width, height) in character cells. Box-drawing characters (like ┌─┐│) create visual borders. The TUI recalculates these when content changes.",
    highlightRegions: [],
  },
  "focus-management": {
    title: "Focus & Input Routing",
    description: "Only one region is 'focused' at a time (shown with a green border). Keystrokes are routed to the focused region. Tab/arrow keys move focus between regions. The cursor position is tracked within the focused region.",
    highlightRegions: ["sidebar"],
  },
  "resize-handling": {
    title: "Terminal Resize",
    description: "When you resize the terminal window, it sends a SIGWINCH signal. The TUI queries the new size with ioctl(), then recalculates every region's dimensions and re-renders the entire screen.",
    highlightRegions: [],
  },
  rendering: {
    title: "Full-Screen Rendering",
    description: "TUIs often use 'alternate screen mode' (CSI ?1049h) for a clean canvas. They position the cursor with escape codes and draw each cell. Double-buffering prevents flicker: draw to memory first, then output all at once.",
    highlightRegions: [],
  },
};
      ...
      >
        ...
        >
          {TABS.map((tab, i) => (
            <button
              key={tab}
              onClick={(e) => { e.stopPropagation(); setActiveTab(i); setFocusedRegion("tabs"); }}
            ...
            <div className="p-1">
              {SIDEBAR_ITEMS.map((item, i) => {
                ...
                return (
                  <div
                    key={item.id}
                    onClick={(e) => { e.stopPropagation(); setSelectedItem(i); setFocusedRegion("sidebar"); }}
          ...
          >
            {showCoordinates && (
              <div className="text-terminal-cyan text-[10px] mb-2">
                Region: content ({sidebarWidth},3) {terminalSize.cols - sidebarWidth}x{terminalSize.rows - 3}
              </div>
            )}
            <div className="text-terminal-fg font-bold mb-2">{selectedDeploy.name}</div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="text-terminal-dim text-sm bg-terminal-bg border border-terminal-border rounded p-3">
            <span className="text-terminal-red">Try it:</span> Click to focus a region.
            Use <kbd className="bg-terminal-highlight px-1 rounded">Tab</kbd> to cycle focus.
            Use <kbd className="bg-terminal-highlight px-1 rounded">↑↓</kbd> in sidebar,
            <kbd className="bg-terminal-highlight px-1 rounded">←→</kbd> in tabs.
          </div>
        </div>
          ...
          <div className="bg-terminal-bg border min-h-[340px] border-terminal-border rounded-lg p-4 space-y-4">
            ...
            {currentStep === "layout-system" && (
              <div className="bg-terminal-highlight rounded p-3 font-mono text-xs space-y-2">
                <div className="text-terminal-dim">// Region data structure</div>
              ...
              <div className="bg-terminal-highlight rounded p-3 space-y-2">
                <div className="text-xs space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Currently focused: <span className="text-terminal-green">{focusedRegion}</span></span>
                  </div>
                  <div className="text-terminal-dim mt-2">
                    Focus determines where keystrokes go. The cursor (if any) lives in the focused region.
                  </div>
                </div>
              </div>
            )}
            ...
            {currentStep === "rendering" && (
              <div className="bg-terminal-highlight rounded p-3 font-mono text-xs space-y-2">
                <div className="text-terminal-dim">// Escape sequences for TUI rendering</div>
                <div className="space-y-1">
                  <div><span className="text-terminal-cyan">\x1b[?1049h</span> <span className="text-terminal-dim">— Enter alternate screen</span></div>
                  <div><span className="text-terminal-cyan">\x1b[2J</span> <span className="text-terminal-dim">— Clear entire screen</span></div>
                  <div><span className="text-terminal-cyan">\x1b[H</span> <span className="text-terminal-dim">— Move cursor to (1,1)</span></div>
                  <div><span className="text-terminal-cyan">\x1b[{"{row}"};{"{col}"}H</span> <span className="text-terminal-dim">— Move cursor to position</span></div>
                  <div><span className="text-terminal-cyan">\x1b[?25l</span> <span className="text-terminal-dim">— Hide cursor while drawing</span></div>
                  <div><span className="text-terminal-cyan">\x1b[?25h</span> <span className="text-terminal-dim">— Show cursor when done</span></div>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="border border-terminal-border rounded-lg p-6 space-y-6">
        <h3 className="text-terminal-red text-sm font-bold">Under the Hood: TUI Architecture</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-terminal-highlight rounded p-4 space-y-2">
            ...
            <p className="text-sm text-terminal-dim">
              The TUI maintains a tree of regions (like a DOM). Each region knows its bounds and can have children. When the container resizes, bounds propagate down the tree.
            </p>
          </div>
          ...
          <div className="bg-terminal-highlight rounded p-4 space-y-2">
            ...
            <p className="text-sm text-terminal-dim">
              Input events (keys, mouse) go to the focused region first. If unhandled, they bubble up. Mouse clicks hit-test against region bounds to determine which region was clicked.
            </p>
          </div>
          ...
          <div className="bg-terminal-highlight rounded p-4 space-y-2">
            ...
            <p className="text-sm text-terminal-dim">
              Each region renders to a buffer (2D array of cells). Buffers merge into a final screen buffer. Only changed cells get written to the terminal, minimizing escape sequences.
            </p>
          </div>
        </div>
        ...
        <div className="bg-terminal-bg border border-terminal-border rounded p-4 space-y-3">
          <div className="text-terminal-red text-sm font-bold">Box Drawing Characters</div>
          ...
          <p className="text-terminal-dim text-xs">
            These Unicode characters create the borders you see in TUIs. They're just regular characters—the terminal renders them like any other text.
          </p>
        </div>
      </div>
...
function CursorPositionDemo() {
  ...
  return (
    <div className="border border-terminal-border rounded-lg p-6 space-y-4">
      <h3 className="text-terminal-red text-sm font-bold">Cursor Positioning</h3>
      <p className="text-terminal-dim text-sm">
        The terminal tracks a single cursor position. TUIs constantly move this cursor using escape sequences to draw in different regions. Click any cell below to see the escape sequence that would move the cursor there.
      </p>

      <div className="flex flex-col md:flex-row gap-6">
        <div className="font-mono text-xs">
          <div className="grid gap-0 border border-terminal-border" style={{ gridTemplateColumns: `repeat(${GRID_COLS}, 1fr)` }}>
        ...
        <div className="flex-1 space-y-3">
          <div className="bg-terminal-highlight rounded p-3 font-mono text-sm">
            <div className="text-terminal-dim text-xs mb-2">Escape sequence to move cursor:</div>
            <div className="text-terminal-cyan">\x1b[{cursorPos.row};{cursorPos.col}H</div>
          </div>
          <div className="bg-terminal-highlight rounded p-3 font-mono text-sm">
          ...
          <p className="text-terminal-dim text-xs">
            Position ({cursorPos.row}, {cursorPos.col}) — Row {cursorPos.row}, Column {cursorPos.col}.
            Terminal coordinates are 1-indexed (row 1, col 1 is top-left).
          </p>
        </div>
      </div>
    </div>
