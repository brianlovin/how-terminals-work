    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="flex-1 space-y-6">
          <div>
            <label className="block text-terminal-dim text-sm mb-2">Character</label>
          ...
          <div>
            <label className="block text-terminal-dim text-sm mb-2">Foreground Color</label>
            <div className="flex flex-wrap gap-2">
              {(Object.keys(COLORS) as Array<keyof typeof COLORS>).map((color) => (
                <button key={color} onClick={() => setFg(color)}
                  ...
                  style={{ backgroundColor: COLORS[color] }} title={color} />
              ))}
            </div>
          </div>

          <div>
            <label className="block text-terminal-dim text-sm mb-2">Background Color</label>
            <div className="flex flex-wrap gap-2">
              {(Object.keys(BG_COLORS) as Array<keyof typeof BG_COLORS>).map((color) => (
                <button key={color} onClick={() => setBg(color)}
                  ...
                  style={{ backgroundColor: color === "none" ? "#0d1117" : BG_COLORS[color] }} title={color} />
              ))}
            </div>
          </div>

          <div>
            <label className="block text-terminal-dim text-sm mb-2">Style Attributes</label>
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={bold} onChange={(e) => setBold(e.target.checked)} className="w-4 h-4 accent-terminal-green" />
                <span className={bold ? "font-bold" : ""}>Bold</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={underline} onChange={(e) => setUnderline(e.target.checked)} className="w-4 h-4 accent-terminal-green" />
