...
const NERD_FONT_ICONS = [
  ...
  { icon: "git", name: "Git", codepoint: "U+F1D3", category: "Dev", color: "text-terminal-red" },
  { icon: "github", name: "GitHub", codepoint: "U+F408", category: "Dev", color: "text-terminal-fg" },
...
const FILE_TREE: { name: string; type: "folder" | "file"; icon: IconName; color: string; indent: number; codepoint: string }[] = [
  ...
  { name: "README.md", type: "file", icon: "markdown", color: "text-terminal-fg", indent: 0, codepoint: "U+E73E" },
  { name: "Dockerfile", type: "file", icon: "docker", color: "text-terminal-cyan", indent: 0, codepoint: "U+F308" },
];
...
const STEPS: Record<ExplainerStep, { title: string; description: string }> = {
  what: {
    ...
    description: "Modern terminal apps like file explorers, status bars, and dev tools display icons for files, folders, and status indicators. These aren't images—they're Unicode characters rendered by special fonts.",
  },
  how: {
    title: "How They Work",
    description: "Terminal icons are just regular Unicode characters. The terminal treats them exactly like letters or numbers—one character per cell. The magic is in the font, which maps these codepoints to icon glyphs.",
  },
  pua: {
    title: "The Private Use Area",
    description: "Unicode reserves ranges (U+E000-U+F8FF) called Private Use Areas. Nerd Fonts place thousands of icons here—dev logos, file types, git symbols, and more. Apps output these codepoints; fonts render them as icons.",
  },
  fonts: {
    title: "Nerd Fonts",
    description: "Nerd Fonts are regular programming fonts (like JetBrains Mono or Fira Code) patched with 3,600+ icons. Install one, set it as your terminal font, and icons just work. No terminal configuration needed.",
  },
  rendering: {
    title: "The Rendering Pipeline",
    description: "When an app outputs an icon: 1) It prints a Unicode character (e.g., U+E628 for TypeScript). 2) The terminal looks up the character in its font. 3) The font maps U+E628 to a TypeScript logo glyph. 4) The terminal draws that glyph in a cell.",
  },
};
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="space-y-4">
          <div className="text-sm text-terminal-dim">File Explorer with Icons</div>
            ...
            <div className="font-mono text-sm min-h-[280px]">
              ...
              <div className="flex items-center gap-4 mb-3 pb-2 border-b border-terminal-border">
                ...
                <span className="text-terminal-dim text-xs">
                  {showWithIcons ? "Simulating Nerd Font glyphs" : "Plain text only"}
                </span>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-2 px-2 py-0.5 rounded cursor-pointer transition-colors ${
                        isHovered ? "bg-terminal-green/20" : "hover:bg-terminal-border/50"
                      }`}
                    ...
                    >
                      ...
                      <span className={isHovered ? "text-terminal-green" : "text-terminal-fg"}>
                        {item.name}
                      </span>
                    </div>
                  );
              ...
              {hoveredFile !== null && (
                <div className="mt-4 pt-3 border-t border-terminal-border text-xs">
                  <div className="text-terminal-dim mb-2">This icon is a single Unicode character:</div>
          ...
          <div className="bg-terminal-bg border min-h-[260px] border-terminal-border rounded-lg p-4 space-y-4">
            ...
            {currentStep === "pua" && (
              <div className="bg-terminal-highlight rounded p-3 font-mono text-xs space-y-2">
                <div className="text-terminal-dim">// Unicode Private Use Area ranges</div>
                <div className="space-y-1">
                  <div><span className="text-terminal-cyan">U+E000 - U+F8FF</span> <span className="text-terminal-dim">— Basic Multilingual Plane PUA</span></div>
            ...
            {currentStep === "rendering" && (
              <div className="bg-terminal-highlight rounded p-3 space-y-3">
                <div className="flex flex-wrap items-center gap-3 text-sm">
                  ...
                  <div className="flex flex-col items-center">
                    <span className="text-terminal-cyan font-mono text-xs">Font lookup</span>
                    <span className="text-terminal-green">Nerd Font</span>
                  </div>
                  <span className="text-terminal-dim">→</span>
                  <div className="flex flex-col items-center">
                    <span className="text-terminal-cyan font-mono text-xs">Rendered</span>
                    <div className="w-6 h-6">{renderIcon("typescript", "text-terminal-blue")}</div>
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="border border-terminal-border rounded-lg p-6 space-y-6">
        <h3 className="text-terminal-red text-sm font-bold">Nerd Font Icon Gallery</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            {categories.map(category => (
              <div key={category} className="space-y-2">
                <div className="text-terminal-dim text-xs uppercase tracking-wide">{category}</div>
          ...
          <div className="bg-terminal-highlight rounded-lg p-4 space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-terminal-fg font-bold">{selectedIcon.name}</div>
                <div className="text-terminal-cyan font-mono text-sm">{selectedIcon.codepoint}</div>
              </div>
            </div>

            <div className="space-y-3">
              <div className="bg-terminal-bg rounded p-3 font-mono text-xs">
                <div className="text-terminal-dim mb-1">// In shell (with Nerd Font)</div>
                <div className="text-terminal-green">echo -e "\u{selectedIcon.codepoint.replace("U+", "").toLowerCase()} {selectedIcon.name}"</div>
              </div>
              ...
              <div className="bg-terminal-bg rounded p-3 font-mono text-xs">
                ...
                <div className="text-terminal-fg">
                  Codepoint: <span className="text-terminal-cyan">{selectedIcon.codepoint}</span>
                </div>
                <div className="text-terminal-fg">
                  Decimal: <span className="text-terminal-cyan">{parseInt(selectedIcon.codepoint.replace("U+", ""), 16)}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Technical Deep Dive */}
      <div className="border border-terminal-border rounded-lg p-6 space-y-6">
        <h3 className="text-terminal-red text-sm font-bold">Under the Hood: Why One Cell?</h3>
        ...
        <div className="bg-terminal-bg border border-terminal-border rounded p-4 space-y-3">
          <div className="text-terminal-red text-sm font-bold">Character Width in Terminals</div>
          <div className="font-mono text-sm space-y-2">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-terminal-dim text-xs">2 cells each (CJK)</span>
            </div>
          </div>
...
function IconSetsReference() {
  const iconSets = [
    {
      ...
      range: "U+E0A0-E0D4",
      icons: [
        { render: (c: string) => <svg viewBox="0 0 24 24" fill="currentColor" className={`w-4 h-4 ${c}`}><path d="M0 0l12 12L0 24V0z"/></svg> },
        { render: (c: string) => <svg viewBox="0 0 24 24" fill="currentColor" className={`w-4 h-4 ${c}`}><path d="M12 0l12 12-12 12V0z"/></svg> },
        { render: (c: string) => <svg viewBox="0 0 24 24" fill="currentColor" className={`w-4 h-4 ${c}`}><path d="M24 0L12 12l12 12V0z"/></svg> },
      ],
    },
    {
      name: "Font Awesome",
  ...
  return (
    <div className="border border-terminal-border rounded-lg p-6 space-y-4">
      <h3 className="text-terminal-red text-sm font-bold">Icon Sets in Nerd Fonts</h3>
      <p className="text-terminal-dim text-sm">
        ...
        {iconSets.map((set) => (
          ...
          >
            <div className="md:w-32">
              <div className="text-terminal-fg font-bold text-sm">{set.name}</div>
              <div className="text-terminal-cyan font-mono text-xs">{set.range}</div>
            </div>
            <div className="text-terminal-dim text-xs flex-1">{set.description}</div>
